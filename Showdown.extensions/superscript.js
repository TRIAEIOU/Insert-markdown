...
(function (extension) {
  ...
  function htmlSupToMarkdown(evt) {
    if (evt.input.nodeName !== 'SUP') {
      evt.output = null;
      return evt;
    }
    let txt = '';
    // resolve children
    evt.input.childNodes.forEach(child => {
      txt += showdown.subParser('makeMarkdown.node')(child, evt.globals);
    });
  ...
  showdown.extension('superscript', function() {
    ...
    return {
      ...
      listeners: {
        ...
        "makemarkdown.unhandledNode": htmlSupToMarkdown
      }
    };
  });
}));