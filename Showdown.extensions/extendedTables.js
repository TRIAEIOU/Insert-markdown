...
(function (extension) {
  ...
  function markdownTableToHtml(evt) {
     ...
     * HTML to markdown
     */
    evt.output = evt.input;
    patterns.forEach(re => {
      evt.output = evt.output.replace(re.pattern, function (match) {
  ...
  function htmlTableToMarkdown(evt) {
    'use strict';
    let rows = Array.from(evt.input.querySelectorAll('tr'));
  ...
  showdown.extension('extendedTables', function () {
    ...
    return {
      ...
      listeners: {
        ...
        "makemarkdown.table.onCapture": htmlTableToMarkdown
      }
    };
  });
}));